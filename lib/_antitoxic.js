...
handler.before = async function (m, { conn, isAdmin, isBotAdmin, isOwner }) { 
    ...
    const isTossico = regexTossico.exec(m.text)

    if (isTossico && chat.antiTossico && !isOwner && !isAdmin) {
        user.warn += 1
        if (!(user.warn >= 4)) 
            await m.reply(`${user.warn == 1 ? `*@${m.sender.split`@`[0]}*` : `*@${m.sender.split`@`[0]}*`}, hai usato una parola proibita: (${isTossico})... Hai: *${user.warn}/4* avvertimenti.`, false, { mentions: [m.sender] })

        if (user.warn >= 4) {
            user.warn = 0
            await m.reply(`Verrai eliminato \n*@${m.sender.split`@`[0]}*`, false, { mentions: [m.sender] })
            user.banned = true
