let handler = async (m, { conn, groupMetadata, participants, isBotAdmin, isSuperAdmin }) => {
    ...
    conn.on('group-participants-update', async (anu) => {
        try {
            ...
            if (anu.action == 'add') {
                ...
                const ppimg = await conn.profilePictureUrl(num, 'image').catch(_ => 'https://telegra.ph/file/60c9e0f3c3c5ba2e3e1d8.png');
                const teks = `Benvenuto/a @${num.split('@')[0]} nel gruppo *${mdata.subject}*`;
                await conn.sendMessage(mdata.id, { image: { url: ppimg }, caption: teks, mentions: [num] });
            }
        } catch (e) {
            console.error(e);
        }
    ...
    // Invia un messaggio di addio ai membri che lasciano il gruppo
    conn.on('group-participants-update', async (anu) => {
        try {
            const mdata = await conn.groupMetadata(anu.jid);
            if (anu.action == 'remove') {
                const num = anu.participants[0];
                const ppimg = await conn.profilePictureUrl(num, 'image').catch(_ => 'https://telegra.ph/file/60c9e0f3c3c5ba2e3e1d8.png');
                const teks = `Addio @${num.split('@')[0]}, ci mancherai!`;
                await conn.sendMessage(mdata.id, { image: { url: ppimg }, caption: teks, mentions: [num] });
            }
        } catch (e) {
            console.error(e);
